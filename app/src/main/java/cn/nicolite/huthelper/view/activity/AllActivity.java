...
public class AllActivity extends BaseActivity<IBaseView, BaseActivity> {
    ...
    private List<Menu> mainMenuList = new ArrayList<>();//主界面菜单
    private List<Menu> otherMenuList = new ArrayList<>();//其他菜单
    ...
    protected void doBusiness() {
        ...
        adapterMain.setOnEditClickListener(new MenuAdapter.EditClickLister() {
            ...
            public void onEditClick(View v, int position) {
                try{
                    Menu item = mainMenuList.remove(position);//含有越界异常
        ...
        adapterOther.setOnEditClickListener(new MenuAdapter.EditClickLister() {
            @Override
            public void onEditClick(View v, int position) {
                if (mainMenuList.size() >= 20) {
                    showMessage("主页面最多放置20个应用");
                    return;
                }
                try {
                    Menu item = otherMenuList.remove(position);//含有越界异常
                    item.setIsMain(true);
